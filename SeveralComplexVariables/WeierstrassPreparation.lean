...
nonvanishing holomorphic function times a monic polynomial of degree `d`. The lower
...
def weierstrassPolynomial {d : ℕ} (a : Fin d → E → ℂ) (z : E × ℂ) : ℂ :=
...
/-- Local preparation consists of a unit and a monic polynomial whose lower coefficients
vanish at the parameter origin. Equality is equality of germs at the origin. -/
structure IsWeierstrassPreparationAt {d : ℕ} (f u : E × ℂ → ℂ)
    (a : Fin d → E → ℂ) : Prop where
  unit_analytic : AnalyticAt ℂ u 0
  unit_ne_zero : u 0 ≠ 0
  coefficient_analytic : ∀ j, AnalyticAt ℂ (a j) 0
  coefficient_zero : ∀ j, a j 0 = 0
...
theorem IsWeierstrassPreparationAt.division {d : ℕ} {f u : E × ℂ → ℂ}
    ...
    IsWeierstrassDivisionAt f (fun z => z.2 ^ d) (fun z => (u z)⁻¹)
      (fun j x => -a j x) := by
  refine ⟨h.unit_analytic.inv h.unit_ne_zero, fun j => (h.coefficient_analytic j).neg, ?_⟩
  filter_upwards [h.eq, h.unit_analytic.continuousAt.eventually_ne h.unit_ne_zero] with z hz hne
...
theorem weierstrass_preparation_at {d : ℕ} {f : (ι → ℂ) × ℂ → ℂ}
    (hf : AnalyticAt ℂ f 0)
    (horder : analyticOrderAt (fun w : ℂ => f (0, w)) 0 = d) :
  ...
  obtain ⟨q, a, hD, _⟩ := weierstrass_division_at hf hp horder
  ...
  have hDc : IsWeierstrassDivisionAt (fun z : (ι → ℂ) × ℂ => f (0, z.2))
    ...
    refine ⟨hD.quotient_analytic.comp_of_eq hc rfl, fun _ => analyticAt_const, ?_⟩
  ...
  have H : IsWeierstrassPreparationAt f (fun z => (q z)⁻¹) (fun j x => -a j x) := by
    refine ⟨hD.quotient_analytic.inv hq0, inv_ne_zero hq0,
      fun j => (hD.coefficient_analytic j).neg, fun j => by simp [ha0], ?_⟩
    filter_upwards [hD.eq, hD.quotient_analytic.continuousAt.eventually_ne hq0] with z hz hne
...
/-- **Weierstrass preparation (Jakóbczak–Jarnicki 1.7.2).** On a sufficiently small
...
theorem weierstrass_preparation {d : ℕ} {f : (ι → ℂ) × ℂ → ℂ}
    {U : Set ((ι → ℂ) × ℂ)} (hU : IsOpen U) (h0 : 0 ∈ U)
    (hf : DifferentiableOn ℂ f U)
    (horder : analyticOrderAt (fun w : ℂ => f (0, w)) 0 = d) :
    ∃ (r : ι → ℝ) (R : ℝ) (u : (ι → ℂ) × ℂ → ℂ) (a : Fin d → (ι → ℂ) → ℂ),
      (∀ i, 0 < r i) ∧ 0 < R ∧ polydiscWithRadii 0 r ×ˢ ball 0 R ⊆ U ∧
      DifferentiableOn ℂ u (polydiscWithRadii 0 r ×ˢ ball 0 R) ∧
      (∀ z ∈ polydiscWithRadii 0 r ×ˢ ball 0 R, u z ≠ 0) ∧
      (∀ j, DifferentiableOn ℂ (a j) (polydiscWithRadii 0 r)) ∧
      (∀ j, a j 0 = 0) ∧
      EqOn f (fun z => u z * weierstrassPolynomial a z) (polydiscWithRadii 0 r ×ˢ ball 0 R) ∧
      ∀ v b, IsWeierstrassPreparationAt f v b →
        u =ᶠ[𝓝 0] v ∧ ∀ j, a j =ᶠ[𝓝 0] b j := by
  obtain ⟨u, a, H, hu⟩ := weierstrass_preparation_at
    ((hf.analyticOnNhd_finiteDimensional hU) _ h0) horder
  ...
  have hn := hU'.and (H.unit_analytic.eventually_analyticAt.and
    ((H.unit_analytic.continuousAt.eventually_ne H.unit_ne_zero).and (ha.and H.eq)))
  obtain ⟨ε, hε, hball⟩ := Metric.mem_nhds_iff.mp hn
  have hP : polydiscWithRadii (0 : ι → ℂ) (fun _ => ε) ×ˢ ball (0 : ℂ) ε =
      ball (0 : (ι → ℂ) × ℂ) ε := by
    rw [polydiscWithRadii_const_eq_ball _ hε, ball_prod_same]
    rfl
  have hprop := fun z (hz : z ∈ polydiscWithRadii (0 : ι → ℂ) (fun _ => ε) ×ˢ ball (0 : ℂ) ε) =>
    hball (hP ▸ hz)
  refine ⟨fun _ => ε, ε, u, a, fun _ => hε, hε, fun z hz => (hprop z hz).1,
    fun z hz => (hprop z hz).2.1.differentiableAt.differentiableWithinAt,
    fun z hz => (hprop z hz).2.2.1, ?_, H.coefficient_zero,
...
theorem IsWeierstrassPreparationAt.comp_equiv {F : Type*} [NormedAddCommGroup F]
  ...
  refine ⟨h.unit_analytic.comp_of_eq hpair h0,
    by show u (φ 0, (0 : ℂ)) ≠ 0; rw [hφmap]; exact h.unit_ne_zero,
    fun j => (h.coefficient_analytic j).comp_of_eq hφ hφmap,
    fun j => by show a j (φ (0 : F)) = 0; rw [hφmap]; exact h.coefficient_zero j,
...
theorem weierstrass_preparation_at_findim [FiniteDimensional ℂ E] {d : ℕ} {f : E × ℂ → ℂ}
  ...
  obtain ⟨v, b, Hg, huniqg⟩ := weierstrass_preparation_at hgan hgorder
...
theorem IsWeierstrassPreparationAt.germ_factorization {d : ℕ} {f u : E × ℂ → ℂ}
    {a : Fin d → E → ℂ} (h : IsWeierstrassPreparationAt f u a)
    (hf : AnalyticAt ℂ f 0) :
    IsUnit (AnalyticGerm.ofAnalyticAt u h.unit_analytic) ∧
    AnalyticGerm.ofAnalyticAt f hf = AnalyticGerm.ofAnalyticAt u h.unit_analytic *
      AnalyticGerm.ofAnalyticAt (weierstrassPolynomial a)
        (analyticAt_weierstrassPolynomial h.coefficient_analytic) := by
