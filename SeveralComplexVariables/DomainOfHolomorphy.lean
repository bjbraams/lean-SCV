...
Common continuation from `U` to `V` requires agreement only on a specified overlap `W`.
It does not require `U ⊆ V`, or agreement on every component of `U ∩ V`.
`IsDomainOfHolomorphy` excludes such a common continuation outside `U`.
`IsDomainOfExistence` excludes continuation of one specified scalar function.
Openness and connectedness of `U` are separate; the predicates also apply to disconnected
open sets. The whole space and the empty set are included.

References: Jakóbczak–Jarnicki §2.7; Scheidemann §7.2; Boas §4.2.3.
These definitions do not construct Riemann domains or envelopes of holomorphy.
-/
...
/-- All scalar analytic functions on `U` continue to `V`, with agreement on `W`.
Containment and topological assumptions are supplied separately. -/
def HasCommonAnalyticContinuation (U V W : Set E) : Prop :=
  ∀ f : E → ℂ, AnalyticOnNhd ℂ f U →
    ∃ g : E → ℂ, AnalyticOnNhd ℂ g V ∧ EqOn g f W

/-- No common continuation through a nonempty open overlap reaches outside `U`.
For open `U`, this is the domain-of-holomorphy property, without imposing connectedness. -/
...
/-- A scalar function is analytic on `U` and has no continuation beyond it from any
nonempty open overlap. This is the strong, local meaning of domain of existence. -/
...
/-- Entire functions vanishing at each exterior point but nowhere on `U` obstruct all
common continuation outside `U`, by applying the identity theorem to their reciprocals. -/
...
/-- Every subset of the complex plane satisfies the analytic continuation obstruction;
in particular every planar open set is a domain of holomorphy. -/
...
/-- Finite products of planar open sets are domains of holomorphy. The continuation
obstruction itself holds for arbitrary planar factors and includes an empty index type. -/
...
/-- Every real-convex open subset of a finite-dimensional complex normed space is a
 domain of holomorphy. A separating real functional is complexified to give a pole. -/
