...
A divisor of finite order in the distinguished scalar coordinate admits a
unique quotient and a polynomial remainder. We normalize the divisor by a
nonvanishing leading factor, make the remaining perturbation small, and apply
the Picard iteration. Uniqueness holds on smaller neighborhoods and hence for
germs. The uniform statement applies to bounded numerators on a common
polydisc; the local statement applies to arbitrary analytic numerators.
...
Reference: Jakóbczak–Jarnicki, §1.7, Theorem 1.7.3. These analytic statements
do not follow just from Mathlib's formal, adic division theorem.
-/
...
polydisc-ball. This supplies the normalization in `weierstrass_division`. -/
...
omit [Fintype ι] in
/-- Uniform coefficient bounds give a uniform bound for a Weierstrass remainder
on a scalar disc, including the empty remainder when `d = 0`. -/
...
/-- If the remainder coefficients tend to zero and the leading factor is bounded
away from zero, shrinking only the parameter polydisc gives the contraction bound.
The scalar radius and an upper bound for the parameter radius can be prescribed. -/
private theorem exists_small_remainder_div {d : ℕ} {c : Fin d → (ι → ℂ) → ℂ}
    ...
    (hlower : ∀ z ∈ polydiscWithRadii (0 : ι → ℂ) (fun _ => r₀) ×ˢ ball (0 : ℂ) R,
      δ ≤ ‖f1 z‖) :
    ∃ r : ℝ, 0 < r ∧ r ≤ r₀ ∧
      ∀ z ∈ polydiscWithRadii (0 : ι → ℂ) (fun _ => r) ×ˢ ball (0 : ℂ) R,
  ...
  have hw' : ‖w‖ < min s r₀ := by
    simpa [pi_norm_lt_iff (lt_min hs hr₀), mem_polydiscWithRadii] using hw
  ...
  have hl := hlower (w, ζ) ⟨polydiscWithRadii_mono _ (fun _ => min_le_right s r₀) hw, hζ⟩
  rw [norm_div]
  calc
...
`weierstrass_division`. The bound `δ` on `‖f1‖` over a fixed compact set is
...
theorem exists_perturbation_bound_of_coordinatePower_leadingFactor {d : ℕ} {f : (ι → ℂ) × ℂ → ℂ}
  ...
  obtain ⟨r₃, hr₃pos, hr₃ε₁, hhbound⟩ :=
    exists_small_remainder_div hR₂pos hδpos (half_pos hε₁) hcj0'
      (fun z hz => hδle z ⟨mem_closedPolydiscWithRadii.mpr (fun i => (mem_polydiscWithRadii.mp hz.1 i).le),
        ball_subset_closedBall hz.2⟩)

  exact ⟨R₂, δ, r₃, hR₂pos, hR₂ε₁, hδpos, hr₃pos, hr₃ε₁, hhA1, hcj0', hδle,
    fun w hw ζ hζ => hhbound (w, ζ) ⟨hw, hζ⟩⟩

/-- Germ uniqueness for division by a normalized divisor. A fresh small-remainder
estimate allows comparison on any smaller neighborhood supporting the other
decomposition; bounded fixed-point uniqueness then identifies both germs. -/
private theorem normalized_division_germ_unique {d : ℕ} {r₃ R₂ δ : ℝ}
    ...
    (hf1FINAL : DifferentiableOn ℂ f1
      (polydiscWithRadii 0 (fun _ => r₃) ×ˢ ball 0 R₂))
    (hlower : ∀ z ∈ polydiscWithRadii 0 (fun _ => r₃) ×ˢ ball 0 R₂, δ ≤ ‖f1 z‖)
    (hcj0' : ∀ j, Tendsto (c j) (𝓝 0) (𝓝 0))
    (hhFINAL : DifferentiableOn ℂ (fun z => weierstrassRemainder c z / f1 z)
      (polydiscWithRadii 0 (fun _ => r₃) ×ˢ ball 0 R₂))
    (hf_eq2 : ∀ z ∈ polydiscWithRadii 0 (fun _ => r₃) ×ˢ ball 0 R₂,
  ...
  obtain ⟨ρ₀, hρ₀pos, hρ₀sub, hlocal⟩ := hdiv'.exists_divisionOn
    ((isOpen_polydiscWithRadii _ _).prod isOpen_ball) h00
  obtain ⟨ρ, hρpos, hρρ₀⟩ := exists_between hρ₀pos
  have hρsub : polydiscWithRadii (0 : ι → ℂ) (fun _ => ρ) ×ˢ ball (0 : ℂ) ρ ⊆ domFINAL :=
    (Set.prod_mono (polydiscWithRadii_mono _ (fun _ => hρρ₀.le))
  ...
  have hdomsub5 : polydiscWithRadii (0 : ι → ℂ) (fun _ => r₅) ×ˢ ball (0 : ℂ) ρ ⊆ domFINAL :=
    fun z hz => hρsub ⟨polydiscWithRadii_mono _ (fun _ => hr₅ρ) hz.1, hz.2⟩
  set dom5 : Set ((ι → ℂ) × ℂ) := polydiscWithRadii (0 : ι → ℂ) (fun _ => r₅) ×ˢ ball (0 : ℂ) ρ
    with hdom5def
  have hlocalSub : dom5 ⊆ polydiscWithRadii 0 (fun _ => ρ₀) ×ˢ ball 0 ρ₀ :=
    Set.prod_mono (polydiscWithRadii_mono _ (fun _ => hr₅ρ.trans hρρ₀.le))
      (ball_subset_ball hρρ₀.le)
  have hq'Adiff : DifferentiableOn ℂ q' dom5 := hlocal.quotient_holomorphic.mono hlocalSub
  have ha'Adiff : ∀ j, DifferentiableOn ℂ (a' j) (polydiscWithRadii 0 (fun _ => r₅)) :=
    fun j => (hlocal.coefficient_holomorphic j).mono
      (polydiscWithRadii_mono _ (fun _ => hr₅ρ.trans hρρ₀.le))
  ...
  have hSdiv : IsWeierstrassDivisionOn (fun z => z.2 ^ d) (g - hh * S) S aOut
      (polydiscWithRadii (0 : ι → ℂ) (fun _ => r₅)) ρ :=
    ⟨hfixed.quotient_holomorphic.mono hdomsub5,
      fun j => (hfixed.coefficient_holomorphic j).mono hpoly53, fun _ hz => hfixed.eq (hdomsub5 hz)⟩
  -- Compact containment bounds the difference of the two fixed-point candidates.
  let K := closedPolydiscWithRadii (0 : ι → ℂ) (fun _ => ρ) ×ˢ closedBall (0 : ℂ) ρ
  have hclosedsub : K ⊆ polydiscWithRadii 0 (fun _ => ρ₀) ×ˢ ball 0 ρ₀ :=
  ...
  have hcont : ContinuousOn (fun z => S z - s' z) K :=
    (hfixed.quotient_holomorphic.continuousOn.mono hKfinal).sub
      ((hlocal.quotient_holomorphic.continuousOn.mono hclosedsub).mul
...
/-- Undoing the nonvanishing leading factor converts a normalized fixed-point
division into division by the original divisor. The remainder is unchanged. -/
private theorem divisionOn_div_leadingFactor {d : ℕ} {r : ι → ℝ} {R : ℝ}
    {f f1 g h S : (ι → ℂ) × ℂ → ℂ} {a : Fin d → (ι → ℂ) → ℂ}
    (hf1 : DifferentiableOn ℂ f1 (polydiscWithRadii 0 r ×ˢ ball 0 R))
    (hne : ∀ z ∈ polydiscWithRadii 0 r ×ˢ ball 0 R, f1 z ≠ 0)
    (heq : ∀ z ∈ polydiscWithRadii 0 r ×ˢ ball 0 R, f z = f1 z * (z.2 ^ d + h z))
    (hfixed : IsWeierstrassDivisionOn (fun z => z.2 ^ d) (g - h * S) S a
      (polydiscWithRadii 0 r) R) :
    IsWeierstrassDivisionOn f g (fun z => S z / f1 z) a (polydiscWithRadii 0 r) R := by
  refine ⟨?_, hfixed.coefficient_holomorphic, ?_⟩
  · change DifferentiableOn ℂ (fun z => S z / f1 z) _
    simp_rw [div_eq_mul_inv]
    exact hfixed.quotient_holomorphic.mul (hf1.inv hne)
  · intro z hz
...
theorem weierstrass_division {d : ℕ} {f : (ι → ℂ) × ℂ → ℂ}
  ...
  have hfixed : IsWeierstrassDivisionOn (fun z => z.2 ^ d) (g - hh * S) S aOut
      (polydiscWithRadii 0 (fun _ => r₃)) R₂ := by
...
The uniform division theorem supplies the quotient and its germ uniqueness. -/
...
This follows from the uniform division theorem. -/
