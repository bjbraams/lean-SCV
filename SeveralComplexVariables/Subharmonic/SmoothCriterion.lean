...
theorem submeanAt_of_laplacian_pos (hg : ContDiffAt ℝ 2 g t₀) (hΔ : 0 < Δ g t₀) :
    SubmeanAt g t₀ :=
...
theorem SubmeanAt.laplacian_nonneg (hg : ContDiffAt ℝ 2 g t₀) (hs : SubmeanAt g t₀) :
    0 ≤ Δ g t₀ := by
  by_contra hlt
  push Not at hlt
  obtain ⟨r, ⟨_, h₁⟩, ⟨_, h₂⟩⟩ :=
...
theorem subharmonicOn_of_laplacian_nonneg {U : Set ℂ} (hU : IsOpen U) (hg : ContDiffOn ℝ 2 g U)
  ...
  have hsub_ε : SubharmonicOn gε U :=
    ⟨hgε.continuousOn.upperSemicontinuousOn, fun t ht =>
      submeanAt_of_laplacian_pos (hgε.contDiffAt (hU.mem_nhds ht)) (hΔε t ht)⟩
