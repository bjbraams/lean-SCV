...
On open finite-dimensional complex domains, holomorphic finite-coordinate equations
define analytic subsets. Biholomorphic changes of ambient coordinates preserve analytic
subsets. These results use the holomorphy–analyticity equivalence and are separated from
`AnalyticSet.Basic` so that the definition and elementary analytic-set operations do not
import holomorphic mapping theory.
