...
References: Hörmander (1973), Theorem 2.3.2; Jakóbczak–Jarnicki (2021), Theorem 4.2.5;
Boas (2013), Section 2.4.
-/
...
/-- Cutoff data for a compact hole `K` in an open set `D`: a smooth function equal to one on an
open neighborhood `U` of `K`, with open support `s` whose closure is compact in `D`. -/
structure HoleCutoffData (D K : Set (ℂ × G)) where
  /-- The cutoff function. -/
  φ : ℂ × G → ℝ
...
theorem nonempty_holeCutoffData {D K : Set (ℂ × G)} (hD : IsOpen D) (hK : IsCompact K)
...
def holeCutoff (K : Set (ℂ × G)) (φ : ℂ × G → ℝ) (f : ℂ × G → F) (x : ℂ × G) : F :=
...
variable {D K : Set (ℂ × G)} (h : HoleCutoffData D K) {f : ℂ × G → F}

omit [FiniteDimensional ℂ G] in
/-- The modified function is `C²` on `D`. -/
theorem contDiffAt_holeCutoff (hD : IsOpen D) (hKc : IsClosed K)
...
def dbarExt (D : Set (ℂ × G)) (F₀ : ℂ × G → F) (v : ℂ × G) (x : ℂ × G) : F :=
...
theorem dbarExt_holeCutoff_eq_zero (hf : AnalyticOnNhd ℂ f (D \ K)) (v : ℂ × G) {x : ℂ × G}
...
theorem dbarAlong_fderiv_dbarExt_holeCutoff_symm (hD : IsOpen D) (hKc : IsClosed K)
...
theorem exists_analyticOnNhd_extension_of_isCompact_prod {D K : Set (ℂ × G)} (hD : IsOpen D)
  ...
  have hga : AnalyticOnNhd ℂ g D := hdiff.analyticOnNhd_finiteDimensional hD
