...
References: Fritzsche–Grauert (2002), Chapter II, Section 2 ("The Levi Form", Theorem 2.8);
Hörmander (1973), Theorem 2.6.2; Range (1986), Chapter II, Section 2.6.
-/
...
/-- The second derivative of a complex-line slice of a `C²` function is the second derivative
of the function evaluated on the direction vectors. -/
theorem fderiv_fderiv_slice {a w : E} {t₀ : ℂ} (hf : ContDiffAt ℝ 2 f (a + t₀ • w)) (s s' : ℂ) :
  ...
  set φ : ℂ → E := fun t => a + lineCLM w t with hφdef
  have hφ : ∀ t, HasFDerivAt φ (lineCLM w) t := fun t => (lineCLM w).hasFDerivAt.const_add a
  have hφc : Continuous φ := (lineCLM w).continuous.const_add a
  ...
  have hg : (fun t => fderiv ℝ (f ∘ φ) t) =ᶠ[𝓝 t₀]
      fun t => (fderiv ℝ f (φ t)).comp (lineCLM w) := by
  ...
  have hcomp : HasFDerivAt (fun t => fderiv ℝ f (φ t))
      ((fderiv ℝ (fderiv ℝ f) (φ t₀)).comp (lineCLM w)) t₀ := hD2.comp t₀ (hφ t₀)
  have hfin := hcomp.clm_comp (hasFDerivAt_const (lineCLM w) t₀)
...
theorem fderiv_slice {a w : E} {t₀ : ℂ} (hf : DifferentiableAt ℝ f (a + t₀ • w)) :
    fderiv ℝ (fun t : ℂ => f (a + t • w)) t₀ = (fderiv ℝ f (a + t₀ • w)).comp (lineCLM w) := by
  have hφ : HasFDerivAt (fun t : ℂ => a + lineCLM w t) (lineCLM w) t₀ :=
    (lineCLM w).hasFDerivAt.const_add a
...
/-- The complex line through `a` in direction `w` is smooth. -/
theorem contDiff_line (a w : E) : ContDiff ℝ 2 fun t : ℂ => a + t • w := by
  have : (fun t : ℂ => a + t • w) = fun t => a + lineCLM w t := rfl
  rw [this]
  exact contDiff_const.add (lineCLM w).contDiff
...
theorem plurisubharmonicOn_of_leviForm_nonneg (hU : IsOpen U) (hf : ContDiffOn ℝ 2 f U)
  ...
  apply subharmonicOn_of_laplacian_nonneg (hU.preimage (continuous_line a w))
  · exact hf.comp (contDiff_line a w).contDiffOn fun _ ht => ht
  · intro t ht
