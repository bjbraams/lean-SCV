...
Starting from division by a coordinate power, we construct successive
approximations for a small perturbation of that divisor. A geometric error
bound gives a holomorphic limit; convergence of Taylor coefficients identifies
its polynomial remainder. A separate contraction argument proves uniqueness
for the perturbed division equation.

These results supply the iterative step of Jakóbczak–Jarnicki, Theorem 1.7.3.
Normalization of a general divisor is in
...
/-- The geometric error bound for Picard approximations also controls the
remainders uniformly on each scalar slice. -/
private theorem tendstoUniformlyOn_picardRemainder
    ...
    (hSbound : ∀ k, ∀ z ∈ polydiscWithRadii 0 r ×ˢ ball 0 R,
      ‖(picardApprox d r R hr hR h g hg hh k).1 z - S z‖ ≤
        ((d + 1 : ℕ) : ℝ) / R ^ d * M * (1 / 2) ^ k / (1 - 1 / 2))
    {w : ι → ℂ} (hw : w ∈ polydiscWithRadii 0 r) :
    TendstoUniformlyOn
...
/-- A uniform limit of scalar polynomials of degree less than `d` is the
polynomial formed from its first `d` Taylor coefficients. Derivative convergence
identifies the coefficients, and the finite sum then passes to the limit. -/
