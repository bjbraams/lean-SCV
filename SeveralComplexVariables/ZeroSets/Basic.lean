...
/-- A nonzero analytic function on a connected open set is nonzero arbitrarily near each
point of that set. No completeness or finite-dimensionality assumption is needed. -/
...
/-- The nonvanishing locus of a nonzero analytic function is dense in its connected open
domain. The closure is taken in the ambient normed space. -/
...
/-- Continuous extensions across the zero set of a nonzero analytic function are unique
on the domain. Their values outside the domain are unrestricted. -/
...
/-- If a product of two analytic functions vanishes near a point, one factor vanishes near
that point. This is the identity principle on a sufficiently small connected ball. -/
theorem eventuallyEq_zero_or_eventuallyEq_zero_of_mul
    {f g : E → ℂ} {x : E} (hf : AnalyticAt ℂ f x) (hg : AnalyticAt ℂ g x)
  ...
  · push Not at hzero
    ...
    have hgon : AnalyticOnNhd ℂ g (ball x r) := fun z hz => (hball hz).2.1
