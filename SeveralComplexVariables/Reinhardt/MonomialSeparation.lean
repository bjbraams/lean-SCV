...
/-- A face point in an open complete Reinhardt set has a positive lift with the same
nonzero coordinates. -/
...
private theorem nonneg_separating_weights {S : Set (ι → ℝ)}
...
private theorem separating_weight_zero {U : Set (ι → ℂ)} {z : ι → ℂ}
...
private theorem linear_functional_eq_sum (l : (ι → ℝ) →L[ℝ] ℝ) (t : ι → ℝ) :
...
/-- Finitely many strict inequalities with nonnegative real weights persist for suitable
nonnegative integer weights. Zero weights remain zero. -/
...
/-- A monomial separates an exterior point from finitely many positive radius vectors
in an open complete logarithmically convex Reinhardt set. -/
