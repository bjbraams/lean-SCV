...
`eqOn_of_holomorphic_of_eqOn` is [Fritzsche–Grauert][FritzscheGrauert2002] (2002), I.4.10, p. 22,
...
* `eqOn_of_holomorphic_of_eqOn`: **Identity theorem ([Fritzsche–Grauert][FritzscheGrauert2002]
  I.4.10).** Two holomorphic maps on an open, preconnected set agree everywhere if they agree on a
...
theorem eqOn_of_holomorphic_of_eqOn {U V : Set E} (hU : IsOpen U) (hconn : IsPreconnected U)
...
domain. -/
theorem eqOn_zero_of_holomorphic_of_eqOn_zero {U V : Set E} (hU : IsOpen U)
    (hconn : IsPreconnected U)
    {f : E → F} (hf : DifferentiableOn ℂ f U)
    (hV : IsOpen V) (hne : V.Nonempty) (hVU : V ⊆ U) (heq : EqOn f 0 V) :
    EqOn f 0 U :=
  eqOn_of_holomorphic_of_eqOn hU hconn hf (differentiableOn_const 0) hV hne hVU heq
