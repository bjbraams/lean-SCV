...
/-- Given an ideal element regular in the distinguished coordinate, division against its
Weierstrass preparation shows the ideal is finitely generated: every element reduces,
modulo the ideal element, to a polynomial remainder of degree below the order; those
remainders occurring in the ideal form a submodule of the finite free module of
degree-bounded coefficient tuples, finitely generated since the coefficient ring is
Noetherian. -/
theorem ideal_fg_of_orderInLastVariable_ne_top [FiniteDimensional ℂ E]
    [IsNoetherianRing (AnalyticGerm (0 : E))] (J : Ideal (AnalyticGerm (0 : E × ℂ)))
    {g : AnalyticGerm (0 : E × ℂ)} (hgJ : g ∈ J) {d : ℕ} (hd : orderInLastVariable g = d) :
    J.FG := by
  classical
  obtain ⟨⟨u, w⟩, ⟨hwdist, hwdeg, hgeq⟩, -⟩ := existsUnique_preparation g hd
  simp only at hwdist hwdeg hgeq
  have hgu : polynomialHom w = (↑u⁻¹ : AnalyticGerm (0 : E × ℂ)) * g := by
    rw [hgeq, ← mul_assoc, ← Units.val_mul, inv_mul_cancel, Units.val_one, one_mul]
  let M : Submodule (AnalyticGerm (0 : E)) (Fin d → AnalyticGerm (0 : E)) :=
  ...
  · intro h hhJ
    ...
    rw [hwdeg] at hrdeg
    have hreq : polynomialHom r = h - q * (↑u⁻¹ : AnalyticGerm (0 : E × ℂ)) * g := by
      rw [heq, hgu]; ring
    ...
    have hrsum : r = ∑ a : S, f a • remainderOfCoefficients (a : Fin d → AnalyticGerm (0 : E)) := by
      rw [har, ← hf, remainderOfCoefficients_sum]
      refine Finset.sum_congr rfl fun a _ => ?_
      rw [Polynomial.smul_eq_C_mul]
      exact remainderOfCoefficients_smul (f a) (a : Fin d → AnalyticGerm (0 : E))
    have hpolyr : polynomialHom r = ∑ a : S, parameterHom (f a) *
        polynomialHom (remainderOfCoefficients (a : Fin d → AnalyticGerm (0 : E))) := by
      rw [hrsum, map_sum]
      refine Finset.sum_congr rfl fun a _ => ?_
      rw [Polynomial.smul_eq_C_mul, map_mul, polynomialHom_C]
    have hheq : h = q * ((↑u⁻¹ : AnalyticGerm (0 : E × ℂ)) * g) +
    ...
    · apply Ideal.sum_mem
      ...
      exact Finset.mem_coe.mpr (Finset.mem_insert_of_mem
        (Finset.mem_image.mpr ⟨(a : Fin d → AnalyticGerm (0 : E)), a.2, rfl⟩))

/-- Analytic induction step for Noetherianity. A nonzero element becomes regular in the
distinguished coordinate after a linear coordinate change; finite generation transports
back along the induced ring automorphism of the germ ring. -/
theorem ideal_fg_prod [FiniteDimensional ℂ E]
  ...
  · obtain ⟨g, hgI, hg0⟩ := Submodule.exists_mem_ne_zero_of_ne_bot hI0
    ...
    have hΦorder : orderInLastVariable (Φ (ofAnalyticAt g0 hg0A)) = d := by
      ...
      exact hd
    have hFG : (I.map (Φ.toRingEquiv : AnalyticGerm (0 : E × ℂ) →+*
        AnalyticGerm (0 : E × ℂ))).FG :=
      ideal_fg_of_orderInLastVariable_ne_top _ (Ideal.mem_map_of_mem _ hgI) hΦorder
    have hcomap : (I.map (Φ.toRingEquiv : AnalyticGerm (0 : E × ℂ) →+*
        AnalyticGerm (0 : E × ℂ))).comap
          (Φ.toRingEquiv : AnalyticGerm (0 : E × ℂ) →+* AnalyticGerm (0 : E × ℂ)) = I :=
      Ideal.comap_map_of_bijective _ hΦbij
    have hI : I = (I.map (Φ.toRingEquiv : AnalyticGerm (0 : E × ℂ) →+*
        AnalyticGerm (0 : E × ℂ))).map
          (Φ.toRingEquiv.symm : AnalyticGerm (0 : E × ℂ) →+* AnalyticGerm (0 : E × ℂ)) := by
      conv_lhs => rw [← hcomap]
...
polynomial, and irreducibility of the polynomial (hence, given a unique factorization base
ring, its primality) transports back to primality of the germ. -/
theorem prime_of_irreducible_of_baseUFD [FiniteDimensional ℂ E]
    [UniqueFactorizationMonoid (AnalyticGerm (0 : E))]
    {g : AnalyticGerm (0 : E × ℂ)} (hg : Irreducible g) : Prime g := by
  obtain ⟨g0, hg0A, rfl⟩ := exists_rep g
  ...
  have hpolyirr : Irreducible (polynomialHom w) :=
    Irreducible.of_mul_isUnit_left u.isUnit hgeq hΦirr
